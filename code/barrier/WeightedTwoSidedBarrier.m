...
% phi(x) = - sum log(x - lb) - sum log(ub - x)
    ...
    methods
        function o = WeightedTwoSidedBarrier(lb, ub, weight)
            o.n = length(lb);
            assert(all(size(weight) == [o.n 1]));
            o.weight = weight;
            o.update(lb, ub);
        end
        ...
        function grad = gradient(o, x)
            grad = o.weight .* (1./(o.ub-x) - 1./(x-o.lb));
        end
        
        function d = hessian(o, x)
            d = o.weight .* (1./((x-o.lb).*(x-o.lb)) + 1./((o.ub-x).*(o.ub-x))) + o.extraHessian;
        end
        
        function t = tensor(o, x)
            t = -2*o.weight.*(1./((x-o.lb).*(x-o.lb).*(x-o.lb)) - 1./((o.ub-x).*(o.ub-x).*(o.ub-x)));
        end
