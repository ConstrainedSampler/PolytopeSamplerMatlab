...
#ifndef CSPARSE_CS_UPDOWN_H_
#define CSPARSE_CS_UPDOWN_H_
#include "cs.h"

/* sparse Cholesky update/downdate, L*L' + sigma*w*w' (sigma = +1 or -1) */
namespace CSparse {
	template <typename Tv, typename Ti>
	Ti cs_updown(cs<Tv,Ti>* L, Ti sigma, const cs<Tv, Ti>* C, const Ti* parent)
	{
		Ti n, p, f, j, * Lp, * Li, * Cp, * Ci;
		Tv* Lx, * Cx, alpha, beta = 1, delta, gamma, w1, w2, * w, beta2 = 1;
		if (!CS_CSC(L) || !CS_CSC(C) || !parent) return (0);  /* check inputs */
		Lp = L->p; Li = L->i; Lx = L->x; n = L->n;
		Cp = C->p; Ci = C->i; Cx = C->x;
		if ((p = Cp[0]) >= Cp[1]) return (1);        /* return if C empty */
		w = cs_malloc<Tv>(n);            /* get workspace */
