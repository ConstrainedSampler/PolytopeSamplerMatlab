...
#ifndef CSPARSE_CS_DMPERM_H_
#define CSPARSE_CS_DMPERM_H_
#include "cs.h"
#include "cs_fkeep.h"
#include "cs_permute.h"
#include "cs_maxtrans.h"
#include "cs_scc.h"
...
namespace CSparse {
	...
	bool cs_bfs(const cs<Tv, Ti>* A, Ti n, Ti* wi, Ti* wj, Ti* queue,
		const Ti* imatch, const Ti* jmatch, Ti mark)
	{
		Ti* Ap, * Ai, head = 0, tail = 0, j, i, p, j2;
		cs<Tv, Ti>* C;
		...
		C = (mark == 1) ? ((cs<Tv, Ti>*)A) : cs_transpose(A, 0);
	...
	void cs_matched(Ti n, const Ti* wj, const Ti* imatch, Ti* p, Ti* q,
		Ti* cc, Ti* rr, Ti set, Ti mark)
	{
		Ti kc = cc[set], j;
		Ti kr = rr[set - 1];
		for (j = 0; j < n; j++)
	...
	template <typename Ti>
	void cs_unmatched(Ti m, const Ti* wi, Ti* p, Ti* rr, Ti set)
	{
		Ti i, kr = rr[set];
	...
	template <typename Tv, typename Ti>
	bool cs_rprune(Ti i, Ti j, Tv aij, void* other)
	{
		Ti* rr = (Ti*)other;
	...
	template <typename Tv, typename Ti>
	csd<Ti>* cs_dmperm(const cs<Tv, Ti>* A, Ti seed)
	{
		Ti m, n, i, j, k, cnz, nc, * jmatch, * imatch, * wi, * wj, * pinv, * Cp, * Ci,
		...
		if (!jmatch) return (cs_ddone<Tv,Ti>(D, NULL, jmatch, 0));
		...
		cs_bfs<Tv,Ti>(A, n, wi, wj, q, imatch, jmatch, 1);       /* find C1, R1 from C0*/
		ok = cs_bfs<Tv,Ti>(A, m, wj, wi, p, jmatch, imatch, 3);  /* find R3, C3 from R0*/
		if (!ok) return (cs_ddone<Tv, Ti>(D, NULL, jmatch, 0));
		cs_unmatched<Ti>(n, wj, q, cc, 0);                    /* unmatched set C0 */
		cs_matched<Ti>(n, wj, imatch, p, q, cc, rr, 1, 1);    /* set R1 and C1 */
		cs_matched<Ti>(n, wj, imatch, p, q, cc, rr, 2, -1);   /* set R2 and C2 */
		cs_matched<Ti>(n, wj, imatch, p, q, cc, rr, 3, 3);    /* set R3 and C3 */
		cs_unmatched<Ti>(m, wi, p, rr, 3);                    /* unmatched set R0 */
		cs_free(jmatch);
		/* --- Fine decomposition ----------------------------------------------- */
		pinv = cs_pinv(p, m);         /* pinv=p' */
		if (!pinv) return (cs_ddone<Tv,Ti>(D, NULL, NULL, 0));
		C = cs_permute(A, pinv, q, 0);/* C=A(p,q) (it will hold A(R2,C2)) */
		cs_free(pinv);
		if (!C) return (cs_ddone<Tv, Ti>(D, NULL, NULL, 0));
		Cp = C->p;
		...
		if (!scc) return (cs_ddone<Tv, Ti>(D, C, NULL, 0));
		...
		D->nb = nb2;
		cs_dfree<Ti>(scc);
		return (cs_ddone<Tv, Ti>(D, C, NULL, 1));
	}
}
