...
#ifndef CSPARSE_CS_COUNTS_H_
#define CSPARSE_CS_COUNTS_H_
#include "cs.h"
#include "cs_leaf.h"
...
namespace CSparse {
	...
	void init_ata(cs<Tv,Ti>* AT, const Ti* post, Ti* w, Ti** head, Ti** next)
	{
		Ti i, k, p, m = AT->n, n = AT->m, * ATp = AT->p, * ATi = AT->i;
	...
	template <typename Tv, typename Ti>
	Ti* cs_counts(const cs<Tv, Ti>* A, const Ti* parent, const Ti* post, bool ata)
	{
		Ti i, j, k, n, m, J, s, p, q, jleaf, * ATp, * ATi, * maxfirst, * prevleaf,
			* ancestor, * head = NULL, * next = NULL, * colcount, * w, * first, * delta;
		cs<Tv, Ti>* AT;
		if (!CS_CSC(A) || !parent || !post) return (NULL);    /* check inputs */
		m = A->m; n = A->n;
		s = 4 * n + (ata ? (n + m + 1) : 0);
		delta = colcount = cs_malloc<Ti>(n);    /* allocate result */
		w = cs_malloc<Ti>(s);                   /* get workspace */
