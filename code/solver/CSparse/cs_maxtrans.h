...
#ifndef CSPARSE_CS_MAXTRANS_H_
#define CSPARSE_CS_MAXTRANS_H_
#include "cs.h"
#include "cs_randperm.h"

namespace CSparse {
	/* find an augmenting path starting at column k and extend the match if found */
	template <typename Tv, typename Ti>
	void cs_augment(Ti k, const cs<Tv,Ti>* A, Ti* jmatch, Ti* cheap, Ti* w,
		Ti* js, Ti* is, Ti* ps)
	{
		Ti found = 0, p, i = -1, * Ap = A->p, * Ai = A->i, head = 0, j;
	...
	template <typename Tv, typename Ti>
	Ti* cs_maxtrans(const cs<Tv, Ti>* A, Ti seed)  /*[jmatch [0..m-1]; imatch [0..n-1]]*/
	{
		Ti i, j, k, n, m, p, n2 = 0, m2 = 0, * Ap, * jimatch, * w, * cheap, * js, * is,
			* ps, * Ai, * Cp, * jmatch, * imatch, * q;
		cs<Tv,Ti>* C;
		if (!CS_CSC(A)) return (NULL);                /* check inputs */
		n = A->n; m = A->m; Ap = A->p; Ai = A->i;
		w = jimatch = cs_calloc<Ti>(m + n);   /* allocate result */
		...
		if (k == CS_MIN(m, n))              /* quick return if diagonal zero-free */
		{
			...
			return (cs_idone<Tv,Ti>(jimatch, NULL, NULL, 1));
		}
		for (i = 0; i < m; i++) m2 += w[i];
		C = (m2 < n2) ? cs_transpose(A, 0) : ((cs<Tv,Ti>*)A); /* transpose if needed */
		if (!C) return (cs_idone(jimatch, (m2 < n2) ? C : NULL, NULL, 0));
		n = C->n; m = C->m; Cp = C->p;
		jmatch = (m2 < n2) ? jimatch + n : jimatch;
		imatch = (m2 < n2) ? jimatch : jimatch + m;
		w = cs_malloc<Ti>(5 * n);             /* get workspace */
