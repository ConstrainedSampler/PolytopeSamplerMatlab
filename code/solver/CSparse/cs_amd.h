...
#ifndef CSPARSE_CS_AMD_H_
#define CSPARSE_CS_AMD_H_
#include "cs.h"
#include "cs_tdfs.h"
...
namespace CSparse {
	...
	Ti cs_wclear(Ti mark, Ti lemax, Ti* w, Ti n)
	{
		Ti k;
	...
	bool cs_diag(Ti i, Ti j, Tv aij, void* other) { return (i != j); }

	/* p = amd(A+A') if symmetric is true, or amd(A'A) otherwise */
	template <typename Tv, typename Ti>
	Ti* cs_amd(Ti order, const cs<Tv,Ti>* A)  /* order 0:natural, 1:Chol, 2:LU, 3:QR */
	{
		cs<Tv, Ti>* C, * A2, * AT;
		Ti* Cp, * Ci, * last, * W, * len, * nv, * next, * P, * head, * elen, * degree, * w,
		...
		if (!AT) return (NULL);
		m = A->m; n = A->n;
		dense = (Ti)CS_MAX(16, 10 * sqrt((double)n));   /* find dense threshold */
		dense = CS_MIN(n - 2, dense);
		if (order == 1 && n == m)
		{
			C = cs_add<Tv,Ti>(A, AT, 0, 0);          /* C = A+A' */
		}
		else if (order == 2)
		{
		...
		cs_spfree(AT);
		if (!C) return (NULL);
		cs_fkeep(C, &cs_diag<Ti, Tv>, NULL);          /* drop diagonal entries */
		Cp = C->p;
		cnz = Cp[n];
		P = cs_malloc<Ti>(n + 1);     /* allocate result */
		W = cs_malloc<Ti>(8 * (n + 1)); /* get workspace */
		...
		mark = cs_wclear<Ti>(0, 0, w, n);         /* clear w */
		...
		while (nel < n)                         /* while (selecting pivots) do */
		{
			...
			mark = cs_wclear<Ti>(mark, lemax, w, n);  /* clear w if necessary */
			...
			mark = cs_wclear<Ti>(mark + lemax, lemax, w, n);    /* clear w */
			/* --- Supernode detection ------------------------------------------ */
